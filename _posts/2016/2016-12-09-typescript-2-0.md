...
```js
public set identifier(newIdentifier: string) {
    let prefix: string = "my\_prefix\_";
...
let user.identifier = '21'; // uniqueIdentifier has now value 'my\_prefix\_21'
...
```js Comme vous pouvez le voir, on précise explicitement le type utilisé entre chevrons. On peut aussi laisser le compilateur trouver tout seul quel type est utilisé à partir du type des arguments.
...
```sh Ensuite, tapez la commande suivante :
...
```sh Vous aurez en sortie un fichier nommé _file.js_
...
```js Généralement, le fichier de déclaration possède le même nom que la bibliothèque. Mais si ce n’est pas le cas, vous pouvez [chercher le paquet correspondant](https://aka.ms/types) à votre bibliothèque.
